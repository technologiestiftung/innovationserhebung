...
class InteractiveLinePlotter(InteractivePlotter):
    ...
    def create_plot(self):
        ...
        for code in self.config["plot_codes"]:
            ...
            self.plot[code].background_fill_color = self.config["background_fill_color"]
            self.plot[code].border_fill_color = self.config["background_fill_color"]

            # Configure labels in x and y axes
            self.plot[code].xaxis.ticker = x_range
            self.plot[code].yaxis.ticker.desired_num_ticks = 8
            self.plot[code].yaxis.minor_tick_line_color = None
            self.plot[code].yaxis.major_tick_line_color = None
            self.plot[code].xaxis.major_tick_line_color = None
            self.plot[code].yaxis.axis_label_text_font_size = "13pt"

            # Hide x grid
            self.plot[code].xgrid.grid_line_color = None

            # Hide axes
            self.plot[code].xaxis.axis_line_width = self.config["axis"]["axis_line_width"]
            self.plot[code].yaxis.axis_line_width = self.config["axis"]["axis_line_width"]
            self.plot[code].xaxis.axis_line_color = self.config["background_fill_color"]
            self.plot[code].yaxis.axis_line_color = self.config["background_fill_color"]
